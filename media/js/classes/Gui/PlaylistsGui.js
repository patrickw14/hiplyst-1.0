function PlaylistsGui() {
    ...
    this.playlist_template = '<div class="sidebar_view-item">' +
        ...
        '<div class="sidebar_view-subtitle">Tracks: {{tracks}}</div>' +
        '<div class="sidebar_view-buttons delete_playlist" data-id="{{id}}">Remove</div>' +
    '</div>';
...
PlaylistsGui.prototype.loadPlaylists = function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data["status"] == "OK") {
                var html = "";
                _this.ui_add_to_lists.html('<div class="add_to_button" data-list-id="nowplaying">Choose a playlist</div>');
                ...
                _this.ui_playlists_sidebar.html("Error loading");
            }
        },
        beforeSend: function() {
